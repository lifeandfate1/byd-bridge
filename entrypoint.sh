...
echo "[entrypoint] which adb: $(command -v adb || echo 'NOT FOUND')"
adb version || { echo "[entrypoint] ERROR: adb not runnable"; exit 10; }
...
if ! adb devices | grep -q "${PHONE_IP}:${ADB_PORT}.*device"; then
  echo "ERROR: Phone not in 'device' state. Check trust dialog/IP/port." >&2
  exit 3
fi
